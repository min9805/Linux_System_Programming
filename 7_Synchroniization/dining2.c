...
//semaphore를 Philosopher에 건다.
//ChopStick을 동시에 드는 개념, deadlock에 빠지지 않는다
...
void Test(int id)
{
	//자신과 양 옆의 상태를 확인하고 State를 변경과 동시에 sem_post로 집을 수 있게 준비한다
...
void PickUp(int id)
{
	...
	//State HUNGRY와 Test는 critical section
	State[id] = HUNGRY;

	Test(id);

	//Philosopher를 나중에 해제하더라도 어짜피 주변은 젓가락을 집을 수 없기에 순서는 상관없다
	if (sem_post(&Mutex) < 0)  {
		perror("sem_post");
		pthread_exit(NULL);
	}
...
void PutDown(int id)
{
	//내려놓으면서 양 쪽이 배고픈 상태이며 젓가락을 집을 수 있는 지 test한다.
	if (sem_wait(&Mutex) < 0)  {
		perror("sem_wait");
		pthread_exit(NULL);
	}
